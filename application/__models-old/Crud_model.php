...
class Crud_model extends CI_Model
{
    ...
    function select_html($from, $name, $field, $type, $class, $e_match = '', $condition = '', $c_match = '', $onchange = '',$condition_type='single')
    {
        ...
        $multi  = 'no';
        $phrase = $name;
        $style = 'border-color: black; color: black; width: 100%;padding: 0px !important; height:28px !important';
        ...
        $return = '<select name="' . $name . '" onChange="' . $onchange . '(this.value,this)" class="' . $class . '" ' . $other .  'style="' . $style . ' data-placeholder="' . $phrase . '" tabindex="2" data-hide-disabled="true" >';
        if (!is_array($from)) {
            if ($condition == '') {
                if ($from == 'country')
				{
             ...
             if ($name == 'age_range') {
                $return .= '<option value=""><b>AGE</b></option>';
            }elseif ($name == 'country') {
                $return .= '<option value=""><b>COUNTRY</b></option>';
            }elseif ($name == 'profession') {
                $return .= '<option value=""><b>PROFESSION</b></option>';
            }
    ...
    function chats_search($table,$limit,$start,$search,$col,$dir)
    {
        $this->db->select('*', FALSE);
        $this->db->from($table);
        $this->db->like($table.'_message_thread_id',$search)->or_like($table.'.message_from',$search)->or_like($table.'.message_to',$search)->or_like($table.'.message_text',$search)->limit($limit,$start)->order_by($col,$dir);
        $query = $this->db->get();
    ...
    function chats_search_count($table,$search)
    {
        $this->db->select('*', FALSE);
        $this->db->from($table);
        $this->db->like($table.'_message_thread_id',$search)->or_like($table.'.message_from',$search)->or_like($table.'.message_to',$search)->or_like($table.'.message_text',$search);
        $query = $this->db->get();
