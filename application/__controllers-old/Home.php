...
class Home extends CI_Controller
{
	 ...
	 */
        ...
        public function index()
        {
                ...
                // $page_data['premium_members'] = $this->db->order_by('rand()')->get_where('member', array('is_blocked' => 'no','is_closed' => 'no'), $max_premium_member_num)->result();
...
//                print_r( $page_data['get_memberss']);
//                die("here");
                $this->load->view('front/index', $page_data);
            } else {
                redirect(base_url() . 'home/listing', 'refresh');
        ...
        function safety_tips()
        {
                ...
                // $page_data['terms_and_conditions'] = $this->db->get_where('general_settings', array('type' => 'safety_tips'))->row()->value;
        ...
        function profile($para1 = "", $para2 = "", $para3 = "")
        {
                ...
                //         $this->load->view('front/profile/gallery/index');
                        ...
                        if ($_FILES['gallery_image']['name'] !== '') {
                                ...
                                $path = $_FILES['gallery_image']['name'];
        ...
        function gallery_upload($para1)
        {
                ...
                // }
                       ...
                       //check if there's a profile picture inserted, then let it insert gallery pic normally
        $this->db->select('profile_image');
        $this->db->from('member');
        $this->db->where('member_id', $this->session->userdata('member_id'));
        $row = $this->db->get()->row();

        $var = explode(":", $row->profile_image);
        $str = substr($var[2], 1, -3);

        //if there is a dp inserted then insert incoming pic into gallery
        if ($str != "default_thumb.jpg") {
            if ($this->member_permission() == FALSE) {
                redirect(base_url() . 'home/login', 'refresh');
            }
            ...
            if ($para1 == "add") {
                ...
                if ($_FILES['image']['name'] !== '') {
                    ...
                    $ext = '.' . pathinfo($path, PATHINFO_EXTENSION);
                    if ($ext == ".jpg" || $ext == ".JPG" || $ext == ".jpeg" || $ext == ".JPEG" || $ext == ".png" || $ext == ".PNG") {
                        $this->Crud_model->file_up("image", "profile", $member_id, '', '', $ext);
                        $images[] = array('image' => 'profile_' . $member_id . $ext, 'thumb' => 'profile_' . $member_id . '_thumb' . $ext);
                        $data['profile_image'] = json_encode($images);
                        ...
                        if ($para1 == "") {
                                ...
                                if ($this->form_validation->run() == FALSE) {
                                        ...
                                        if ($query->num_rows() > 0) {
                                                ...
                                                $package = $query->row()->package_info;
                                                $var = explode(":", $package);
                                                $str = substr($var[1], 1, -17);

                                                $password = substr(hash('sha512', rand()), 0, 7);
                                                $data['password'] = sha1($password);
                                                if ($this->Email_model->password_reset_email('member', $member_id, $password, $str)) {
                                                        $this->db->where('member_id', $member_id);
                                                        $this->db->update('member', $data);
                                                        recache();
                                                        $this->session->set_flashdata('alert', 'email_sent');
 ...
 public function reason()
    {
        ...
        elseif ($prev_password != $current_password) {
            $msg = 'Invalid Password!';
            echo json_encode($msg);
            return false;
        } else {
            if (isset($_POST['delete'])) {
                ...
                $msg = 'Success! Your account has been deleted';
                echo json_encode($msg);

            }
            if (!isset($_POST['delete'])) {
                $msg = 'Please Press Delete Account!';
                echo json_encode($msg);
            }
        }
    }
