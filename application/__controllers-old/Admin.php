...
class Admin extends CI_Controller
{
	 ...
	 */
                ...
                if ($this->admin_permission() == FALSE) {
                        ...
                        if ($para2 == "list_data") {
							...
							";
                                                ...
                                                $nestedData['member_since'] = "<span style='color:black;'>" . date('m/d/Y', strtotime($member->member_since)) . "</span>";
        ...
        function send_email($para1)
        {
                ...
                echo "test";
                $this->Email_model->do_email($from, $system_name, $email, $query->subject, $query->body);
                //	$this->session->set_flashdata('alert', 'delete');
                ...
                if ($para1 == "") {
                    ...
                    if (!empty($rows)) {
                        ...
                        foreach ($rows as $row) {
                            ...
                            if (!isset($object->display_member) || !isset($obj->display_member)) {

                                $this->db->select('*');
                                $this->db->from('deleted_member');
                                $this->db->where('member_id', $row->message_to);
                                $dltobj = $this->db->get()->row();

                                $this->db->select('*');
                                $this->db->from('deleted_member');
                                $this->db->where('member_id', $row->message_from);
                                $dltobject = $this->db->get()->row();

                                $nestedData['message_to'] = $dltobject ? 'Deleted Member' : 'Deleted Member';
                                $nestedData['message_from'] = $dltobj ? 'Deleted Member' : 'Deleted Member';
                                $nestedData['message_text'] = $row->message_text;
                                $nestedData['message_time'] = date('m/d/Y H:i A', $row->message_time);
                                $nestedData['options'] = "<button data-target='#delete_modal' data-toggle='modal' class='btn btn-danger btn-xs add-tooltip' data-toggle='tooltip' data-placement='top' title='" . translate('delete') . "' onclick='delete_message(" . $row->message_id . ")'><i class='fa fa-trash'></i></button>";
                            ...
                            } elseif (isset($object->display_member) || isset($obj->display_member)) {

                                $message_from = $object->display_member;
                                $message_to = $obj->display_member;
                                ...
                                $nestedData['message_text'] = $row->message_text;
                                $nestedData['message_time'] = date('d/m/Y H:i A', $row->message_time);
                                $nestedData['options'] = "<button data-target='#delete_modal' data-toggle='modal' class='btn btn-danger btn-xs add-tooltip' data-toggle='tooltip' data-placement='top' title='" . translate('delete') . "' onclick='delete_message(" . $row->message_id . ")'><i class='fa fa-trash'></i></button>";
