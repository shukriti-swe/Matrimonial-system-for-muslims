    ...
    $(document).ready(function () {
        ...
        */
        ...
        $(window).bind("resize",function () {
            ...
            $(".chat").css({"height":(viewHeight-89)});
            $('.persons').css({"height":(viewHeight-28)});
        ...
        function printGroupListAppend(groups){
            ...
            for(let i=0;i<groups.length;i++){
                ...
                html += " <li class=\"person\" data-chat=\"person1\" id='sddddddd' data-mecreator="+groups[i].meCreator+" data-group=\""+groups[i].groupId+"\">";
        ...
        function printGroupList(groups){
            ...
            for(let i=0;i<groups.length;i++){
                groupIds.push(groups[i].groupId);
                time[groups[i].groupId]=groups[i].lastActive;
                html += " <li  class='person group_"+groups[i].nstyle+"' data-chat=\"person1\" id='group_"+groups[i].groupId+"' data-mecreator="+groups[i].meCreator+" data-group=\""+groups[i].groupId+"\">";
                groupImages[groups[i].groupId]=groups[i].groupImage;
                html +='<span id="groupImage_'+groups[i].groupId+'">';
                for (j=0;j<groups[i].groupImage.length;j++){
        ...
        function getGroupList(callback) {
			var r_profiler_id=$("#select_profiler_id2").val();
            let settings = {
                "async": true,
                "crossDomain": true,
                "url": "<?php echo base_url('imApi/getGroups?limit=') ?>"+groupLimit+"&start=0&r_profiler_id="+r_profiler_id,
                "method": "GET",
                "headers": {
